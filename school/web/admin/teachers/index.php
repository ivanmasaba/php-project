                 ...
                 if( $num > 0 )
                 {
                   ...
                   while(  $row = $stmt->fetch(PDO::FETCH_ASSOC) )
                   {
                    extract( $row );
                      ...
                      $sql = $db->query("SELECT subj_name FROM subjects WHERE id = '$subj_id' LIMIT 1 ");
                      if( $sub = $sql->fetch(PDO::FETCH_ASSOC)){
                          
                       echo "<td>".$sub['subj_name']."</td>";
                      }
